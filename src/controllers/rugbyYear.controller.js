...
const { trainingService, rugbyYearService } = require('../services');
// const logger = require('../config/logger');

const createRugbyYear = catchAsync(async (req, res) => {
  const rugbyYear = await rugbyYearService.createRugbyYear(req.body);
  res.status(httpStatus.CREATED).send(rugbyYear);
});
